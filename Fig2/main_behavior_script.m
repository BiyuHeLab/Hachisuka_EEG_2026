%% Master Script for Behavior analysis (Fig. 2, Supplementary Fig. 3)
...
% plots Super and Basic Category performance for each condition, 
% by PAS rating (plotsuperandbasic_byPAS.m) 
...
ylabelname{1} = '% Super Correct';
ylabelname{2} = '% Basic Correct';
...
disp(pval_corr);
%%
pval_pasbycorr = C2F_plotsuperandbasic_byPAS(corrbyPAS);
% corrbyPAS stats:
...
for g = 1:4                                
    ...
    s = swarmchart(xPos,individDataPts,30,'black','o', 'MarkerEdgeColor','none','MarkerFaceColor', 'k');
    s.XJitter = 'rand';
    s.XJitterWidth = 0.1;
    s.MarkerFaceAlpha = .7;
end
...
for g = 1:2                 
    for r = 1:3               
        xPos = repmat(g + offsets(r),1,numSubjects);
        individDataPts = vioPts(r, :, g)';
        s = swarmchart(xPos,individDataPts.*100,30,'black','o', 'MarkerEdgeColor','none','MarkerFaceColor', condColors{r});
        s.XJitter = 'rand';
        s.XJitterWidth = 0.1;
        s.MarkerFaceAlpha = .7;
    end
