function ds_stacked=cosmo_stack_ccgp(ds_cell,varargin)
...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    hyperplane_values=get_struct_values(ds_cell, 'hyperplane');
    if isempty(sample_values)
        error('missing field .samples');
    end
    if check
        sample_sizes=get_dimension_sizes(dim, sample_values);
        hyperplane_sizes=get_dimension_sizes(dim, hyperplane_values);
    else
        sample_sizes=[];
        hyperplane_sizes = [];
    end
    ds_stacked.samples=stack_values(dim, sample_values, '.samples', check);
    ds_stacked.hyperplane=stack_values(dim, hyperplane_values, '.hyperplane', check);
