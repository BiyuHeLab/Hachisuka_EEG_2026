function results_map = cosmo_searchlight_dist(ds, nbrhood, measure, varargin)
...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    results_map=cosmo_stack_dist(result_map_cell,2);
...
% run searchlight using the options in worker_opt
    ...
    if isfield(res_cell{1},'dist')
        for i = 1:length(res_cell)
            results_map.dist(:,i) = res_cell{i}.dist;
            results_map.activation_pattern{i} = res_cell{i}.activation_pattern;
            % results_map.distbytrial{i} = res_cell{i}.distbytrial;
        end
    end

    if isfield(res_cell{1},'hypall')
        for i = 1:length(res_cell)
            results_map.hyperplane{i} = res_cell{i}.hypall;
            results_map.activation_pattern{i} = res_cell{i}.activation_pattern;
        end
    end
