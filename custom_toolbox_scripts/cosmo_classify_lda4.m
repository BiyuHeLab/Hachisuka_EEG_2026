function [predicted,dist,hyperplane] = cosmo_classify_lda4(samples_train, targets_train, samples_test, opt)

% This script is to also test for distance values, divided by ||w||.
% Appropriately, for a 2-class scneario.

% This is for calculating PS (parallelism score), across conditions!
...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    % test classifier
    [predicted,dist]=test(model, samples_test);
    hyperplane = model.hyperplane;
    % hyperplane is equivalent the difference in model weights
    % across two classes. Only recalculated as a sanity check.
    % hyperplane = model.class_weight(2,:) - model.class_weight(1,:);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % helper functions
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function model=train(samples_train, targets_train, opt)
        ...
        class_diff = class_mean(2, :) - class_mean(1, :);
        hyperplane = class_cov_reg \ class_diff';

        model=struct();
        model.class_mean = class_mean;
        model.class_offset=class_offset;
        model.class_weight=class_weight;
        model.classes=classes;
        model.hyperplane = hyperplane;

function [predicted,dist]=test(model, samples_test)
    % test LDA classifier
    ...
    class_weight_onevec = diff(class_weight)*-1;
    m1plusm2 = sum(class_mean);
    class_offset_onevec = class_weight_onevec * m1plusm2';
    class_proj2=bsxfun(@plus,-.5*class_offset_onevec,class_weight_onevec*samples_test');
    dist = class_proj2;
