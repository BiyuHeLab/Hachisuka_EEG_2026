...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    if contains(params.output, 'weight')
        [pred,accuracy,weight]=cosmo_crossvalidate_weight(ds,classifier,...
                                            partitions,params);
    else
        [pred,accuracy,distall,hypall,activation_pattern]=cosmo_crossvalidate(ds,classifier,...
                                            partitions,params);
    end
    ...
    switch output
        ...
        case 'balanced_accuracy_withweight'
            ds_sa.samples=compute_balanced_accuracy_weight(pred,ds.sa.targets);
        ...
        case 'balanced_accuracy'
            ...
            ds_sa.sa.labels={'balanced_accuracy'};
            ds_sa.hypall=hypall;
            ds_sa.activation_pattern = activation_pattern;
        ...
        case 'fold_dist'
            ...
            [ds_sa.samples,ds_sa.sa.folds]=compute_fold_accuracy(pred,...
                                                        ds.sa.targets);
            [disttemp,~,~]=compute_fold_predictions(distall,ds.sa.targets);
            ds_sa.dist = disttemp;
            % for fold = 1:size(distall,2)
            %     temp = distall(:,fold);
            %     distall2{fold} = temp(~isnan(temp));
            % end
            % ds_sa.distbytrial = distall2;
            ds_sa.activation_pattern = activation_pattern;
...
function [accs,folds,act_pattern]=compute_fold_accuracy(pred,targets)
...
function [fold_pred,folds,fold_targets]=compute_fold_dist(dist2bound,...
                                                                targets)
    nfolds=size(pred,2);
...
function [fold_pred,folds,fold_targets]=compute_fold_predictions(pred,...
    ...
    fold_pred=cellfun(@nanmean,pred_cell);
    folds=cellfun(@nanmean,fold_cell);
    fold_targets=cellfun(@nanmean,target_cell);
...
function acc=compute_balanced_accuracy_weight(pred,targets)
    [class_idx,classes]=cosmo_index_unique(targets);
    nclasses=numel(classes);
