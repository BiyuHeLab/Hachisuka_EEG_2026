...
% ci95 = bootci(1000,@mean,ds_group.samples);
%% Stats
...
% ds_z = cosmo_montecarlo_cluster_stat(ds_group,nbrhood,'h0_mean',0.5,'niter',1000);
...
for s = 1:n_subj
    % temp(s,:) = group_mvpaoutput2{s}.samples(indstart:indend);
...
% thresh = 0.05;
% for t = 1:241
%     [binary(t),pval(t)] = ttest(allbalacc{c,1}(:,t),allbalacc{c,2}(:,t));
% end
% orig_binary = double(pval < thresh | pval > (1-thresh));
...
% % corrsigtimeall = timeval(logical(binary));
...
% 
% end
% legend([plotnum(1),plotnum(2),plotnum(3)],{'Exemplar','Basic','Super'});
end
...
errorbar(1,avgpeaktime(c,1), avgpeaktime(c,1) - CIbyCond_onset{c}(1,1),CIbyCond_onset{c}(1,2) - avgpeaktime(c,1),'.','Color','k','Linewidth',1);
errorbar(2,avgpeaktime(c,2), avgpeaktime(c,2) - CIbyCond_onset{c}(2,1),CIbyCond_onset{c}(2,2) - avgpeaktime(c,2),'.','Color','k','Linewidth',1);
errorbar(3,avgpeaktime(c,3), avgpeaktime(c,3) - CIbyCond_onset{c}(3,1),CIbyCond_onset{c}(3,2) - avgpeaktime(c,3),'.','Color','k','Linewidth',1);
xlim([0 5]);
ylim([-0.4 0.8])
xticks(1:5);
xticklabels({'Super','Basic','Exemplar'});
